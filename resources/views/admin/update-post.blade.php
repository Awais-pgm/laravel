                    ...
                    <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                        <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                            <div class="p-6 bg-white border-b border-gray-200">
                                <form method="POST" action="/update/blog/post/{{$post->id}}" enctype="multipart/form-data">
                                    @csrf
                                    <div class="mb-6">
                                        <label class="block">
                                            ...
                                            <input type="text" name="title"
                                                ...
                                                placeholder="" value="{{$post->post_title}}" />
                                        </label>
                                        @error('title')
                                    ...
                                    <div class="mb-6">
                                        <img src="blog-post-images/{{$post->post_image}}" alt="{{$post->post_image}}" width="300px">
                                    </div>
                                    <div class="mb-6">
                                        <label class="block">
                                            ...
                                            <textarea id="markdown-editor" class="block w-full mt-1 rounded-md @error('description') border-red-500 @enderror" name="description"
                                                rows="3">{{$post->post_description}}</textarea>
                                        </label>
                                        @error('description')
                                        <div class="text-sm text-red-600">{{ $message }}</div>
                                        @enderror
